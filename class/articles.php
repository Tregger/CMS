...
class articles {
    ...
    public function get_articles(){
        ...
        $res=array();
        $i=0;
        while($page = mysql_fetch_array($articles)){
            $i++;
            $res[$i]['id']=$page['id'];
...
public function add_page($data){
    ...
    $sql="
         INSERT INTO
         articles(name, fdescr, url, ord, date, meta, description ) 
         VALUES 
    ...
    public function get_page_by_id($id){
        $sql="Select * from articles where id=".$id." order by ord";
        $articles=mysql_query($sql); 
        if(!$articles){
        ...
        $sql="Select * from articles where url like '".$url."' order by ord";
        $articles=mysql_query($sql); 
        if(!$articles){
