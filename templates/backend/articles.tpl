...
{/literal}
<a id="add_page" title="Добавить статью"
   ...
   href="/admin.php?module=articles&option=add"> 
   Добавить статью
</a>
<br><br><br>
...
<table style=" width:100%;" id="example">
    ...
    {foreach from=$articles item=page}
    <tr>
        <td>{$page.name}</td>
        <td>{$page.url}</td>
        ...
        <td>
            ...
            <a id="edt_page_{$page.id}" title="Редактировать {$page.name}"
   onclick="
   {literal}
   ...
   href="/admin.php?module=articles&option=edit&id={/literal}{$page.id}"
   > 
        ...
        <td>
             <a onclick=" {literal}
                if(confirm('Удалить {/literal} {$page.name} {literal}?'))
                 $.get('/admin.php?id={/literal}{$page.id}{literal}&module=articles&option=delete', function(data){window.location.reload();}); else return false;
                {/literal}
